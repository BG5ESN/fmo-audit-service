...
基于 EMQX 的客户端审计监控：按呼号排行总流量、按主题（FMO/RAW）统计发包量、时间轴反查伪造数据包、服务器健康监控、黑名单管控（拉黑/解封/留痕）。
...
4. **拉黑**：排行榜 / 主题统计 → 呼号行「拉黑」→ 填原因 + 封禁时长（永久或临时，到期自动解除）→ 立即踢下线并禁止重连（调用 EMQX 黑名单 API）
5. **黑名单页**：查看当前生效黑名单（含到期时间）、一键解封、操作历史（谁在何时拉黑/解封了谁、原因）
